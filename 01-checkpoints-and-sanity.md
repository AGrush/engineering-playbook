...
10. If I touched `packages/ai/**`: did evals run and pass? Did I change a prompt without versioning it? Did I add a new tool without an auth check on the handler?
11. If I added a new query on a growable table: is the `EXPLAIN ANALYZE` plan in the PR description? Did I use `SELECT *` (no — name the columns)?
12. If I added or changed shared data (column, FK, entity, status): did I trace consumers (grep readers)? Admin/staff UI using operator-scoped accessors, not public/catalog fetchers? At least one observable check per audience this change serves — not only DB assertions? Any audience out of scope this PR: consumer-matrix row **deferred (Phase N)**, owning phase in `13-master-build-plan.md`, matching **code quality watchlist** entry until Phase N runs? (§00 §3, §22, `00-…md` §5 watchlist)
...
- ❌ **Incomplete vertical slice** on a data contract change (new column, FK, entity, or status). Building the producer path (validation → persistence) without tracing every **consumer**: for each **audience × surface** that reads or selects this data, confirm the correct scoped read path exists. Pre-existing admin/guest/mobile surfaces that "look done" are the usual miss. Unit tests on the writer alone do not close the slice. Deferring a reader for a later PR without a consumer-matrix row **deferred (Phase N)**, an owning phase in `13-master-build-plan.md`, and a matching **code quality watchlist** entry until Phase N runs is the same failure mode as skipping the reader silently. See `00-genesis-and-build-cadence.md` §3 (template) and §4.3 (loop); watchlist: `00-…md` §5.
- ❌ **Generic data-access names** that hide audience or visibility rules (`fetchProducts`, `getOrders`, `loadUsers`). Names must encode **who** and **what subset** (`listProductsForPublicCatalog`, `listProductsForStaffEditor`, `listOrdersForScopedOperator`). Reusing a public/catalog fetcher in staff or admin UI because the name sounds neutral is hidden coupling. (§00 §3)
- ❌ **Staff or admin UI wired to public/end-user data sources** (e.g. a form dropdown backed by a published-only or anonymous catalog query). Operator surfaces need operator-scoped accessors: broader lifecycle states (draft, internal), assignment/tenant scope, and often different RLS. Unless product intent is explicitly "admins only pick published rows," document that in the phase verification. (§00 §3)
- ❌ **Verification that only asserts database state** (FK column non-null, row exists) when the feature's value is **what a human sees** on a given surface. Checklists must include observable checks per audience the phase or PR serves now; audiences deferred per §23 item 12 still need matrix **deferred (Phase N)**, owning phase, and watchlist — not DB-only proof they were "handled." (§00 §3)
