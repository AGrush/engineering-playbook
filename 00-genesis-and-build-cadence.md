...
13. **Design system and styling approach** — the playbook standard UI stack is non-negotiable by default: **Tailwind CSS v4 + shadcn/ui (Radix primitives) + Motion (Framer Motion v12) + Lucide React + `next/font`**. This is the most modern, maintainable, hireable-for stack in 2026: accessible by default, AI-friendly, you own the component code, no version lock-in, consistent patterns across teams. Do not substitute MUI, Mantine, Ant Design, or "build from scratch" without a specific constraint that makes the standard impossible. If the human has an existing codebase with a different library, note it as a constraint in Q9 instead. Three things to confirm:
    - *Design fidelity expectation:* MVP/functional (structured and usable, not pixel-perfect) vs polished product (pixel-perfect, micro-interactions, motion, dark mode) vs somewhere in between? This determines whether a dedicated visual polish phase is added to the plan and how detailed the design system domain doc needs to be.
    - *Brand tokens:* Are brand colors, typography scale, and spacing already defined (Figma variables, a style guide, a brand kit)? Or does the AI propose a palette based on the product vision and industry? Either way, this becomes `packages/design-tokens/` in Phase 1 — locked before any feature work begins.
    - *Existing design assets:* Any Figma files, component snapshots, brand guidelines, or competitor references the AI should reference when writing the design system domain doc and screen map? If yes, ask for links or pasted content now.
...
**UI stack rows (CSS, Component primitives, Animation, Icons, Fonts) default to Adopt.** Substitute only if Q9 (Hard constraints) documented a specific reason that makes the standard impossible — an existing codebase, a client mandate, a non-React platform. Preference alone is not a reason. These rows should almost always read "Adopt" in ADR-001.
...
| Does the project have any user-facing UI (web and/or mobile)? — almost always yes | `07-design-system.md` | `09-appendix.md` (UI stack rows: Tailwind v4, shadcn/ui, Motion, Lucide, `next/font`) + `07-standards-integrations.md` §17 (tokens/constants) + project Q13 answers (fidelity, brand tokens, design assets) |
...
| Canonical UI stack (Tailwind v4, shadcn/ui in `packages/ui/`, Motion via `motion/react`, Lucide only, `next/font`), design tokens, three-layer styling model, no prohibited libs | `008-styling-design.mdc` | `09-appendix.md` UI stack rows + `07-standards-integrations.md` §17 + project `07-design-system.md` — copy full rule body from playbook §16 `008` template verbatim | `alwaysApply: true` (styling violations occur in all files) |
